# main.py — ICT SMART PRO v3.0 | PRODUCTION READY, RAILWAY UYUMLU
import base64
import logging
import asyncio
...
# CORE IMPORTS — Sadece core.py'de mevcut olanlar
from core import (
    initialize, cleanup,
...
try:
    ...
    logging.getLogger("main").warning("indicators.py bulunamadı. Sinyal üretimi sınırlı.")
...
# ==================== EKSİK GLOBAL DEĞİŞKENLER (core.py'de yok, burada tanımlıyoruz) ====================
...
async def ws_signal(websocket: WebSocket, pair: str, timeframe: str):
    ...
    single_subscribers.setdefault(channel, set()).add(websocket)
...
<body>
    ...
    <script>
        ...
        function getSymbol() {{ let pair = document.getElementById('pair').value.trim().toUpperCase(); if (!pair.endsWith("USDT")) pair += "USDT"; return "BINANCE:" + pair; }}
        function createWidget() {{
            const symbol = getSymbol();
            const interval = tfMap[document.getElementById('tf').value] || "5";
            if (tvWidget) tvWidget.remove();
            tvWidget = new TradingView.widget({{autosize:true,height:500,symbol:symbol,interval:interval,timezone:"Etc/UTC",theme:"dark",style:"1",locale:"tr",container_id:"tradingview_widget",studies:["RSI@tv-basicstudies","MACD@tv-basicstudies"]}});
            tvWidget.onChartReady(() => {{
                document.getElementById('status').innerHTML = "✅ Grafik yüklendi • Sinyal bağlantısı kurun";
                setInterval(() => {{
                    try {{ const price = tvWidget.activeChart().getSeries().lastPrice(); if (price && price !== currentPrice) {{ currentPrice = price; document.getElementById('price-text').innerHTML = '$' + parseFloat(price).toFixed(price > 1 ? 2 : 6); }} }} catch(e) {{}}
                }}, 1500);
            }});
        }}
        ...
        async function analyzeChartWithAI() {{
            ...
            btn.disabled = true; btn.innerHTML = "Analiz ediliyor..."; box.style.display = 'block'; comment.innerHTML = "📸 Grafik yakalanıyor...<br>🧠 Analiz yapılıyor...";
            try {{
                const symbol = getSymbol().replace("BINANCE:", "");
                const timeframe = document.getElementById('tf').value;
                const response = await fetch('/api/analyze-chart', {{method: 'POST',headers: {{'Content-Type': 'application/json'}},body: JSON.stringify({{symbol,timeframe}})});
                const data = await response.json();
                comment.innerHTML = data.analysis ? data.analysis.replace(/\\n/g, '<br>') : "❌ Analiz alınamadı: " + (data.detail || 'Bilinmeyen hata');
            }} catch (err) {{ comment.innerHTML = "❌ Bağlantı hatası.<br>" + err.message; }}
            finally {{ btn.disabled = false; btn.innerHTML = "🤖 GRAFİĞİ GPT-4o İLE ANALİZ ET"; }}
        }}
        function connect() {{
            let symbol = document.getElementById('pair').value.trim().toUpperCase(); if (!symbol.endsWith("USDT")) symbol += "USDT";
            ...
            tvWidget = new TradingView.widget({{autosize:true,height:500,symbol:tvSymbol,interval:interval,timezone:"Etc/UTC",theme:"dark",style:"1",locale:"tr",container_id:"tradingview_widget",studies:["RSI@tv-basicstudies","MACD@tv-basicstudies"]}});
            tvWidget.onChartReady(() => {{
                document.getElementById('status').innerHTML = `✅ Grafik yüklendi: ${{symbol}} ${{tfSelect.toUpperCase()}} • Canlı sinyal akışı başladı!`;
                setInterval(() => {{ try {{ const price = tvWidget.activeChart().getSeries().lastPrice(); if (price && price !== currentPrice) {{ currentPrice = price; document.getElementById('price-text').innerHTML = '$' + parseFloat(price).toFixed(price > 1 ? 2 : 6); }} }} catch(e) {{}} }}, 1500);
            }});
            if (ws) ws.close();
            ws = new WebSocket((location.protocol==='https:'?'wss':'ws')+'://'+location.host+'/ws/signal/'+symbol+'/'+tfSelect);
            ws.onopen = () => document.getElementById('status').innerHTML = `✅ ${{symbol}} ${{tfSelect.toUpperCase()}} için canlı sinyal akışı başladı!`;
            ws.onmessage = e => {{
                const d = JSON.parse(e.data);
                ...
                details.innerHTML = `<strong>${{d.pair || symbol.replace('USDT','/USDT')}}</strong><br>Skor: <strong>${{d.score || '?'}}/100</strong> | ${{d.killzone || ''}}<br>${{d.last_update ? 'Son: ' + d.last_update : ''}}<br><small>${{d.triggers || ''}}</small>`;
...
<body>
    ...
    <script>
        ...
        function connect() {{
            ...
            ws.onmessage = e => {{
                ...
                table.innerHTML = data.map((sig, i) => `
                    <tr>
                        ...
                        <td class="${{sig.signal && sig.signal.includes('ALIM') ? 'green' : sig.signal && sig.signal.includes('SATIM') ? 'red' : ''}}">${{sig.signal || 'Bekle'}}</td>
                        <td>${{sig.score || '?'}}/100</td>
            ...
            ws.onerror = () => document.getElementById('status').innerHTML = "❌ WebSocket bağlantı hatası";
            ws.onclose = () => document.getElementById('status').innerHTML = "🔌 Bağlantı kapandı. Yeniden bağlanmak için zaman dilimi seçin.";
        }}
    </script>
</body>
</html>"""
...
async def analyze_chart(request: Request):
    try:
        ...
        # indicators.py varsa gerçek sinyal, yoksa fallback
        analysis = "Sinyal sistemi aktif değil." if not generate_ict_signal else "Gerçek analiz çalışıyor."
